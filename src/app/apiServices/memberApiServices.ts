...
class MemberApiService {
  ...
  public async loginRequest(login_data: any) {
    try {
      ...
      console.log("state::::::: LOGIN::::", result.data.state);
  ...
  public async signupRequest(signup_data: any) {
    try {
      ...
      console.log("state::::::: SIGNUP USER::::", result.data.state);
      assert.ok(result?.data, Definer.general_err1);
      assert.ok(result?.data?.state !== "fail", result?.data?.message);
  ...
  public async memberLikeTarget(data: any) {
    try {
      ...
      assert.ok(result?.data, Definer.general_err1);
      assert.ok(result?.data?.state !== "fail", result?.data?.message);
      console.log("LIKEN MEMBER DATA STATE:", result.data.data);
  ...
  public async updateMemberData(data: MemberUpdateData) {
    try {
      ...
      console.log("state::::::: uploadImageToServer :::", result.data.state);
      assert.ok(result?.data, Definer.general_err1);
      assert.ok(result?.data?.state !== "fail", result?.data?.message);
      const member: Member = result.data.data;
      localStorage.setItem("member_data", JSON.stringify(member));
