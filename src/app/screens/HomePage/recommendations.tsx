...
/** REDUX SELECTOR */
const bestBoArticlesRetriever = createSelector(retrieveBestBoArticles, (bestBoArticles) => ({
	bestBoArticles,
}));

const trendBoArticlesRetriever = createSelector(retrieveTrendBoArticles, (trendBoArticles) => ({
	trendBoArticles,
}));

const newsBoArticlesRetriever = createSelector(retrieveNewsBoArticles, (newsBoArticles) => ({
	newsBoArticles,
}));
	...
	return (
		<div className="top_article_frame">
			<Container maxWidth="lg" sx={{ mb: '50px', mt: '60px' }} style={{ position: 'relative' }}>
                ...
                const art_article_url = article?.art_image ? `${serverApi}/${article?.art_image}`.replaceAll('\\','/'): "/community/default_article.svg";
                return (
                  <Stack className={'article_box'} key={article._id}>
                        ...
                        />
											<span className={'authot_username'}>{article?.member_data.mb_nick}
                      </span>
										</div>
